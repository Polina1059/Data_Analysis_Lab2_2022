...
import pandas as pd
from models import catboost_regr_model, linear_regr_model
...
@click.argument('output_linear_regr_model_filepath', type=click.Path())

@click.argument('output_val_data_filepath', type=click.Path())
@click.argument('output_val_target_filepath', type=click.Path())

def main(input_data_filepath, input_target_filepath, output_catboost_regr_model_filepath, output_linear_regr_model_filepath, 
output_val_data_filepath, output_val_target_filepath):
    ...
    linear_regr_model.fit(train_data, train_target)
    save_as_pickle(linear_regr_model, output_linear_regr_model_filepath)
